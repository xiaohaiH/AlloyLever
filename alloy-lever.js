...
; (function (root, factory) {
        ...
        console[item] = function () {
            ...
            if (item === 'error') {
                if (arguments.length > 1) {
                    report(AlloyLever.settings.reportUrl, { type: 'custom-error', data: [].slice.apply(arguments) })
                }
                else {
                    var a = arguments[0];
                    var params = {
                    ...
                    report(AlloyLever.settings.reportUrl, params);
                }
            }
    ...
    window.onerror = function (msg, url, line, col, error) {
        ...
        var reportPrefix = ss.reportPrefix ? '[' + ss.reportPrefix + ']' : '';
        var params = ss.otherReport || {};
        params[ss.reportKey] = reportPrefix + newMsg;
        params.t = Date.now();

        ss.reportUrl && report(ss.reportUrl, params);
